...
吸附后按 rate_hz 把 grasp_target_cube 锁到 hand_tcp（等）下，默认爬升/返航/降落期间
只要夹爪保持闭合（jp 未高于 open_above），方块会一直跟着机体。可选
detach_after_closed_enable：方块被吸到夹爪中心后，等待夹爪真正闭合一小段时间即解除吸附，
后续由 Gazebo 接触力夹持。
...
参数：
  ...
  ~detach_after_closed_enable true：吸附后 jp<detach_after_closed_below 持续 detach_after_closed_sec 即解除吸附
  ~detach_after_closed_below  上述闭爪判据；默认沿用 gripper_closed_below
  ~detach_after_closed_sec    闭爪压紧后延迟多久解除吸附，建议 0.3~0.8s
...
class GazeboFakeBlueAttach(object):
    def __init__(self):
        ...
        self.detach_after_closed_enable = bool(
            rospy.get_param("~detach_after_closed_enable", False)
        )
        self.detach_after_closed_below = float(
            rospy.get_param("~detach_after_closed_below", self.closed_below)
        )
        self.detach_after_closed_sec = float(
            rospy.get_param("~detach_after_closed_sec", 0.45)
        )
        ...
        self._detach_closed_since = None
    ...
    def _on_tick(self, _evt):
        ...
        try:
            ...
            if self._attached:
                ...
                if self._attached:
                    if (
                        self.detach_after_closed_enable
                        and jp < float(self.detach_after_closed_below)
                    ):
                        dcb = max(0.0, float(self.detach_after_closed_sec))
                        if self._detach_closed_since is None:
                            self._detach_closed_since = now
                        elif now - self._detach_closed_since >= dcb:
                            self._attached = False
                            self._close_since = None
                            self._open_prox_since = None
                            self._detach_open_since = None
                            self._detach_closed_since = None
                            rospy.loginfo(
                                "[FAKE_ATTACH] 闭爪压紧 %.2fs 后解除吸附→转真实接触夹持 | EN: detach after closed grip",
                                dcb,
                            )
                            return
                    else:
