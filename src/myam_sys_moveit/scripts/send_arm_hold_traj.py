#!/usr/bin/env python3
"""启动后向 arm_controller / gripper_controller 发 home 单点轨迹，并可解除 Gazebo 暂停。"""
import time
...
def main():
    ...
    duration = rospy.get_param("~move_duration", 6.0)
    ...
    gazebo_model_name = rospy.get_param("~gazebo_model_name", "myuam")
    urdf_param_name = rospy.get_param("~urdf_param_name", "/myam_arm_description")
    ...
    hold_republish_seconds = rospy.get_param("~hold_republish_seconds", 3.0)
    hold_publish_rate_hz = rospy.get_param("~hold_publish_rate_hz", 20.0)
    controller_manager_ns = rospy.get_param("~controller_manager_ns", "/controller_manager")
    wait_controllers_timeout = rospy.get_param("~wait_controllers_timeout", 60.0)
    ...
    # 解暂停前在「暂停」中发轨迹通常无效且拖长 PX4 poll timeout；默认 0，改为解暂停后高频率维持
    pre_unpause_hold_seconds = rospy.get_param("~pre_unpause_hold_seconds", 0.0)
    hold_rate_hz_while_paused = rospy.get_param("~hold_rate_hz_while_paused", 100.0)
    hold_point_duration = rospy.get_param("~hold_point_duration", 0.05)

    arm_positions = rospy.get_param("~hold_positions", [0.0, 1.05, 1.57, 0.0, 0.0])
    ...
    if unpause_gazebo:
        try:
            ...
            rospy.logerr(
                "若出现 Connection reset：通常是 gzserver 已崩溃（请向上翻日志查找 "
                "gzserver: ... OgreAxisAlignedBox 或 Assertion）。此时与 unpause 无关，需先修仿真崩溃。"
            )
